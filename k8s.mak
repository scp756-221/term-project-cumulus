...
loader: dynamodb-init $(LOG_DIR)/loader.repo.log cluster/loader.yaml
	...
	tools/build-configmap.sh gatling/resources/books.csv cluster/book-header.yaml | kubectl -n $(APP_NS) apply -f -
	tools/build-configmap.sh gatling/resources/checkout.csv cluster/checkout-header.yaml | kubectl -n $(APP_NS) apply -f -
...
$(LOG_DIR)/dynamodb-init.log: cluster/cloudformationdynamodb.json
	...
	$(AWS) cloudformation create-stack --stack-name db-anan786 --template-body file://$< || true | tee $(LOG_DIR)/dynamodb-init.log
...
#
dynamodb-clean:
	$(AWS) cloudformation delete-stack --stack-name db-anan786 || true | tee $(LOG_DIR)/dynamodb-clean.log
